...
def get_rinema_file():
  ...
  movie_data_out_df["Title"] = movie_data_df["nAma"]

  def get_review(row):
    row = row.fillna('')
    review = " ".join([str(row['viShayaH']), str(row['tolana-TippaNiH'])])
    return review.strip()
  ...
  movie_data_out_df.to_csv(os.path.join(LOCAL_DATA_DIR_PATH, "movie_data_rinema.tsv"), sep=',')
